...
class RandomTargetMasking:
    '''
    '''

    def __init__(self, p=0.1, type=1):
        assert 0 <= p <= 1.0
        assert int(type) in [1, 2, 3]
        self.p = p
        self.type = int(type)
    ...
    def __call__(self, labels):
        if self.p < 0.01:
            return labels
        # print(img.shape)
        if 'img_ori' not in labels:
            labels['img_ori'] = deepcopy(labels['img'])
            labels['cls_ori'] = deepcopy(labels['cls'])
            labels['instances_ori'] = deepcopy(labels['instances'])
        else:
            labels['img'] = deepcopy(labels['img_ori'])
            labels['cls'] = deepcopy(labels['cls_ori'])
            labels['instances'] = deepcopy(labels['instances_ori'])
        ...
        # random select target and remove it
        remove_idxs = []

        for i in range(len(cls)):
            if np.random.rand() < self.p and cls[i][0] in [0, 1, 3, 5, 7]:
                remove_idxs.append(i)
                # print(cls[i][0])

        selected_bboxes = bboxes[remove_idxs]
        for i in range(len(selected_bboxes)):
            x1, y1, x2, y2 = selected_bboxes[i].astype(np.int32)
            avg_color = np.mean(img, axis=(0, 1)).astype(np.uint8)
            center_x = (x1 + x2) // 2
            center_y = (y1 + y2) // 2

            random_way = np.random.randint(4)
            if self.type == 2:
                if random_way == 0:
                    img[y1:center_y, x1:x2] = avg_color
                elif random_way == 1:
                    img[center_y:y2, x1:x2] = avg_color
                elif random_way == 2:
                    img[y1:y2, x1:center_x] = avg_color
                elif random_way == 3:
                    img[y1:y2, center_x:x2] = avg_color
            elif self.type == 1:
                if random_way == 0:
                    img[y1:center_y, x1:center_x] = avg_color
                elif random_way == 1:
                    img[y1:center_y, center_x:x2] = avg_color
                elif random_way == 2:
                    img[center_y:y2, x1:center_x] = avg_color
                elif random_way == 3:
                    img[center_y:y2, center_x:x2] = avg_color
            elif self.type == 3:
                if random_way == 0:
                    img[y1:center_y, center_x:x2] = avg_color
                    img[center_y:y2, x1:x2] = avg_color
        ...
        bboxes = np.delete(bboxes, remove_idxs, axis=0)
        cls = np.delete(cls, remove_idxs, axis=0)
...
class MixUpPlus(BaseMixTransform):

    def __init__(self, dataset, pre_transform=None, p=0.0, class_probs=None, img_labels=None):
        super().__init__(dataset=dataset, pre_transform=pre_transform, p=p)
        assert class_probs is not None, "class_probs must be provided"
        assert img_labels is not None, "img_labels must be provided"
        self.class_probs = class_probs
        self.img_labels = img_labels
        self.cls2imgidx = {}
        for i, img_labels_i in enumerate(self.img_labels):
            for cls in img_labels_i:
                if cls not in self.cls2imgidx:
                    self.cls2imgidx[cls] = set()
                self.cls2imgidx[cls].add(i)

        self.sorted_cls_idx = sorted(self.class_probs, key=lambda x: self.class_probs[x], reverse=True)

    def get_indexes(self):
        """Get a random index from the dataset based on class probabilities."""
        chosen_cls = np.random.choice(list(self.class_probs.keys()), p=list(self.class_probs.values()))
        chosen_cls_idx = self.sorted_cls_idx.index(chosen_cls)
        new_sorted_cls_idx = self.sorted_cls_idx[chosen_cls_idx:] + self.sorted_cls_idx[:chosen_cls_idx][::-1]
        indexes = []
        for cls in new_sorted_cls_idx:
            img_ids = list(self.cls2imgidx[cls])
            indexes.append(np.random.choice(img_ids))
        return indexes
    ...
    def _mix_transform(self, labels):
        ...
        for ml in mix_labels:
            set1 = set(labels['cls'].flatten().tolist()) & set([3,5,7])
            set2 = set(ml['cls'].flatten().tolist()) & set([3,5,7])
            if not set1 & set2:
                labels2 = ml
                break
...
def compute_class_probs(dataset):
    ...
    for i in range(len(dataset)):
        ...
        img_labels_i = set()
        for cls in labels['cls']:
            if cls[0] in [0, 1, 3, 5, 7]:
                class_counts[cls[0]] += 1
                img_labels_i.add(cls[0])
        img_labels.append(img_labels_i)

    # calculate the inverse frequency of each class
    inverse_freq = {cls: 1.0 / count for cls, count in class_counts.items()}
    
    # calculate the probability of each class
    total = sum(inverse_freq.values())
    class_probs = {cls: freq / total for cls, freq in inverse_freq.items()}
    
    return class_probs, img_labels

def v8_transforms_plus(dataset, imgsz, hyp, stretch=False):
    """Convert images to a size suitable for YOLOv8 training."""

    class_probs, img_labels = compute_class_probs(dataset)

    pre_transform = Compose([
        RandomTargetMasking(p=hyp.rmt, type=hyp.rmt_type),
        CutOff(p=hyp.cutoff),
        Mosaic(dataset, imgsz=imgsz, p=hyp.mosaic),
        CopyPaste(p=hyp.copy_paste),
        RandomPerspective(
    ...
    return Compose([
        pre_transform,
        MixUpPlus(dataset, pre_transform=pre_transform, p=hyp.mixup, class_probs=class_probs, img_labels=img_labels) if hyp.use_mixupplus else MixUp(dataset, pre_transform=pre_transform, p=hyp.mixup),
        Albumentations(p=1.0),
        RandomHSV(hgain=hyp.hsv_h, sgain=hyp.hsv_s, vgain=hyp.hsv_v),
        RandomFlip(direction='vertical', p=hyp.flipud),
        RandomFlip(direction='horizontal', p=hyp.fliplr, flip_idx=flip_idx)])  # transforms
